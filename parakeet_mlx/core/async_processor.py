...
class AsyncAudioBuffer:
    """Asynchronous audio buffer for real-time processing"""
    ...
    async def put(self, item: mx.array) -> bool:
        ...
        async with self._not_full:
            if len(self._buffer) >= self.max_size:
                # Buffer is full, wait or drop oldest
...
class AsyncTranscriptionProcessor:
    """Asynchronous transcription processor for real-time audio"""
    
    def __init__(
        self,
        model: Any,  # BaseParakeet model
    ...
    async def _process_chunk(self, audio_chunk: mx.array, chunk_id: int) -> ProcessingResult:
        ...
        try:
            ...
            if time.time() - self._last_stats_time > 10.0:  # Every 10 seconds
...
class AsyncStreamingTranscriber:
    """High-level async interface for streaming transcription"""
    ...
    async def transcribe_stream(
        ...
        async for audio_chunk in audio_stream:
            ...
            while True:
                result = await self.get_result(timeout=0.001)  # Very short timeout
                if result is None:
                    break
                yield result
    
    def get_current_result(self) -> Optional[AlignedResult]:
        """Get current transcription result (non-blocking)"""
