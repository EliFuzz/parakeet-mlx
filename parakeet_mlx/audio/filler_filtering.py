...
class FillerWordFilter:
    """
    High-performance filler word filter for real-time transcription
    """
    ...
    def is_filler_word(self, text: str) -> bool:
        ...
        # Skip empty or very short text unless specifically configured
        if len(normalized_text) < self.config.min_word_length and not self.config.filter_single_letters:
            return False

        # Don't filter meaningful single letters (like "I" or "a" in proper context)
        if len(normalized_text) == 1:
            # "I" should never be filtered
            if original_text == "I":
                return False
            # "a" should only be filtered if it's clearly a vocalization, not an article
            if normalized_text == "a":
                # For now, be conservative and don't filter "a"
                return False
        ...
        # Check for incomplete words (ending with hyphen)
    ...
    def filter_result(self, result: AlignedResult) -> AlignedResult:
        """
        Filter filler words from a transcription result
        
        Args:
            result: Original transcription result
            
        Returns:
            AlignedResult: Filtered result with filler words removed
        """
        ...
        # Rebuild text from filtered sentences
    ...
    def _filter_sentence(self, sentence: AlignedSentence) -> Optional[AlignedSentence]:
        """Filter filler words from a sentence"""
        filtered_tokens = []
        ...
        # Rebuild sentence text from filtered tokens
    ...
    def filter_text_only(self, text: str) -> str:
        """
        Quick text-only filtering for immediate output scenarios
        
        Args:
            text: Input text to filter
            
        Returns:
            str: Filtered text with filler words removed
        """
