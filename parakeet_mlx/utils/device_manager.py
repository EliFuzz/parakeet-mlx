...
class DeviceManager:
    ...
    def find_device(self, identifier: Union[int, str], refresh: bool = False) -> Optional[AudioDevice]:
        ...
        Args:
            identifier: Device ID (int) or name (str)
            refresh: Whether to refresh the device list
    ...
    def test_device(self, device: Union[AudioDevice, int], duration: float = 1.0) -> bool:
        """
        Test if a device is working by recording for a short duration
        
        Args:
            device: AudioDevice object or device ID
        ...
        try:
            # Try to record for the specified duration
            recording = sd.rec(
...
def find_audio_device(identifier: Union[int, str], refresh: bool = False) -> Optional[AudioDevice]:
    """
    Find audio device by ID or name.
...
def get_recommended_device() -> Optional[AudioDevice]:
    """
    Get a recommended audio device for transcription
    
    Returns:
        Recommended AudioDevice or None if not available
    """
    ...
    # Prefer devices with higher sample rates and more channels
    def device_score(device: AudioDevice) -> float:
        ...
        # Prefer certain device types (heuristic based on name)
    ...
    # Sort devices by score and return the best one
